...
def mesh2sdf_csv(mesh_filename, args, verbose = False):
    assert os.path.isfile(mesh_filename), {'The input file does not exist. Please Verify the path ' + args.path_to_data + '.'}
    
    dir_name = os.path.dirname(mesh_filename)
    mesh_file = os.path.basename(mesh_filename)
    ...
    print('Original mesh loaded from ' + mesh_filename + '.')
...
if __name__ == "__main__":
    ...
    mesh2sdf_csv(args.path_to_data, args, verbose=True)
